...
def main():
    ...
    svgDiagram.save(svgDiagram)
    print("wrote", svgFilename)

# 这里实现的一个接口函数，传入参数是某一个工厂对象，返回参数是一个工厂生产出来的复杂商品
# 除非工厂生产商品的公共流程发生变化，否则这个地方不需要修改（对修改封闭）
...
# 这是一个文本工厂类（也是一个基类）
# 这个类对它提供的是一堆加工过程的函数接口，包括 生产一个框架，生产一个符号矩形，生产一个字符串
# 注意，这个类并不提供功能叫做保存，因为这个保存的功能是框架的功能，在Diagram类中实现
...
# 这是一个SVG工厂类（也是一个子类），和基类一样，流程一样，但是加工实际过程不一样
# 这就是抽象工厂模式的好处：对功能扩展开放，对接口修改封闭；比如，可以随时增加一个XXXDiagram，只要也实现三个相同的流程，就可以快速扩展
